...
public class CopyOfPaypalActivity extends Activity implements OnClickListener {
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		layoutSplitPayment.setLayoutParams(
				new LayoutParams(
						LayoutParams.WRAP_CONTENT, 
						LayoutParams.WRAP_CONTENT));
		...
		layoutChainedPayment.setLayoutParams(
				new LayoutParams(
						LayoutParams.WRAP_CONTENT, 
						LayoutParams.WRAP_CONTENT));
		...
		layoutPreapproval.setLayoutParams(
				new LayoutParams(
						LayoutParams.WRAP_CONTENT, 
						LayoutParams.WRAP_CONTENT));
	...
	public void setupButtons() {
		...
		// The CheckoutButton is an android LinearLayout 
		// so we can add it to our display like any other View.
		layoutSplitPayment.addView(launchParallelPayment);
		
		// Get the CheckoutButton. There are five different sizes. 
		// The text on the button can either be of type TEXT_PAY or TEXT_DONATE.
		launchChainedPayment 
			= pp.getCheckoutButton(
					this, 
					PayPal.BUTTON_194x37, 
					CheckoutButton.TEXT_PAY);
		...
		// Show our labels and the pre-approval EditText.
	...
	private PayPalAdvancedPayment exampleParallelPayment() {
    	...
    	// Create the PayPalReceiverDetails. 
    	// You must have at least one of these to make 
    	// an advanced payment and you should have
    	// more than one for a Parallel or Chained payment.
		PayPalReceiverDetails receiver1 = new PayPalReceiverDetails();
		
		// Sets the recipient for the PayPalReceiverDetails. 
		// This can also be a phone number.
		receiver1.setRecipient("example-merchant-2@paypal.com");
		
		// Sets the subtotal of the payment for this receiver, 
		// not including tax and shipping amounts. 
    	...
    	// Sets the Custom ID. This is any ID that you would 
    	// like to have associated with the PayPalReceiverDetails.
    	receiver1.setCustomID("001813");
    	
    	// Add the receiver to the payment. Alternatively, 
    	// you can create an ArrayList<PayPalReceiverOptions>
	...
	private PayPalAdvancedPayment exampleChainedPayment() {
    	...
    	// Create the PayPalReceiverDetails. You must have at least 
    	// one of these to make an advanced payment and you should have
    	// more than one for a Parallel or Chained payment.
		PayPalReceiverDetails receiver1 = new PayPalReceiverDetails();
		...
		// Sets the primary flag for the receiver. 
		// One receiver must be a primary to create a Chained payment.
    	...
    	// Sets the Custom ID. This is any ID that you 
    	// would like to have associated with the PayPalReceiverDetails.
	...
	private PayPalPreapproval examplePreapproval() {
    	...
    	// Sets the memo. This memo will be part 
    	// of the notification sent by PayPal to the necessary parties.
	...
	public void onClick(View v) {
		...
		if(v == launchSimplePayment) {
			...
			// Set our preapproval key. In order to start a preapproval, 
			// you will need a preapproval key.  In order to
			// get this key, you will need to make a call externally to the library. 
			// Our application uses a simple
			// EditText for the key to be entered into.
			PayPal.getInstance()
				.setPreapprovalKey(
						enterPreapprovalKey.getText().toString());
			
			// Use peapprove to create our Intent.
			Intent preapproveIntent 
				= PayPal.getInstance().preapprove(
						preapproval, this, new ResultDelegate());
